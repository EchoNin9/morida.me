...
- Invokes `mo-thumb` Lambda async (`InvocationType=Event`) for each item with direct invocation payload
...
- Attached ffmpeg layer to `mo-thumb` Lambda alongside existing Pillow layer in Terraform
