# Enable Google Sign-In (Sign in with Google) for MO

This guide walks through enabling **Sign in with Google** for morida.me using AWS Cognito as the identity broker. MO already uses Cognito for email/password auth; adding Google adds a federated identity provider (IdP).
...
| **MO SPA** | Redirects to Cognito Hosted UI; handles callback with tokens |
...
- AWS Cognito User Pool (MO already has this)
- Cognito domain configured (e.g. `mo-auth.auth.us-east-1.amazoncognito.com`)
...
2. Create a new project or select an existing one (e.g. "MoRida")
...
3. Fill required fields:
   - **App name:** MoRida (or your app name)
   - **User support email:** your email
   - **Developer contact:** your email
4. **Authorized domains** — **critical:**
   - Click **Add domain**
   - Add **`amazoncognito.com`** (required for Cognito Hosted UI)
   - If you use a custom Cognito domain (e.g. `auth.morida.me`), add **`morida.me`** as well
5. Save
...
4. **Name:** e.g. "MO Cognito"
5. **Authorized JavaScript origins:**
   - `https://mo-auth.auth.us-east-1.amazoncognito.com` (replace with your Cognito domain)
   - For local dev: `http://localhost:5173` (or your Vite port)
   - For staging: `https://stage.morida.me`
   - For production: `https://morida.me`
6. **Authorized redirect URIs:**
   - `https://mo-auth.auth.us-east-1.amazoncognito.com/oauth2/idpresponse`
...
1. **Cognito → User pools → your pool (mo-user-pool)**
...
| Setting | Value |
|---------|-------|
| **Callback URL(s)** | `https://morida.me/`, `https://stage.morida.me/`, `http://localhost:5173/` (dev) |
...
allowed_oauth_flmo_user_pool_client = true
...
const domain = 'mo-auth.auth.us-east-1.amazoncognito.com';  // from config
...
3. Store tokens (same format as `auth.js` uses: `mo_access_token`, etc.).
...
| Custom Cognito domain: not adding your app domain | If using `auth.morida.me`, add `morida.me` to Authorized domains. |
...
| Wrong Cognito domain | Use your pool’s domain prefix and region (e.g. `mo-auth.auth.us-east-1.amazoncognito.com`). |
| Wrong callback URL in app client | Callback URLs in Cognito must match where you redirect after login (e.g. `https://morida.me/`). |
...
1. **Cognito → User pools → your pool → App integration**
   - Confirm domain: `mo-auth.auth.us-east-1.amazoncognito.com`)
   - Confirm app client has callback URLs and Google enabled.

2. **Test Hosted UI directly:**
   ```
   https://mo-auth.auth.us-east-1.amazoncognito.com/login?client_id=YOUR_CLIENT_ID&response_type=code&redirect_uri=https://morida.me/&scope=openid+email+profile
   ```
...
## Reference: MO-specific values

| Item | Value |
|------|-------|
| Cognito domain prefix | `mo-auth` (from `cognitoDomainPrefix` variable) |
| Region | `us-east-1` |

Construct full domain: `https://mo-auth.auth.us-east-1.amazoncognito.com`
