...
resource "aws_iam_role" "githubStaging" {
  name               = "github-actions-morida-staging"
...
resource "aws_iam_role" "githubProduction" {
  name               = "github-actions-morida-production"
...
data "aws_iam_policy_document" "deploy" {
  ...
  statement {
    ...
    actions = ["lambda:*"]
    resources = [
      "arn:aws:lambda:${var.awsRegion}:${data.aws_caller_identity.current.account_id}:function:mo-*",
      "arn:aws:lambda:${var.awsRegion}:${data.aws_caller_identity.current.account_id}:layer:mo-*",
      "arn:aws:lambda:${var.awsRegion}:${data.aws_caller_identity.current.account_id}:layer:mo-*:*"
    ]
  }
...
resource "aws_cognito_user_pool" "main" {
  ...
  verification_message_template {
    ...
    email_subject        = "MoRida - Verify your email"
...
resource "aws_lambda_layer_version" "ffmpeg" {
  ...
  layer_name          = "mo-ffmpeg-layer"
  compatible_runtimes = ["python3.12"]
}

resource "aws_iam_role" "lambdaApi" {
  name = "mo-api-lambda-role"
