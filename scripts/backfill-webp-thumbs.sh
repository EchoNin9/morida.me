...
# mo-thumb Lambda (async) for each image to generate WebP thumbnails.
#
# Usage:
...
TABLE_NAME="${TABLE_NAME:-mo-main}"
BUCKET_NAME="${BUCKET_NAME:-mo-media-${AWS_ACCOUNT_ID:?AWS_ACCOUNT_ID env var must be set (or override BUCKET_NAME directly)}}"
THUMB_FUNCTION="${THUMB_FUNCTION:-mo-thumb}"
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  echo "Errors:                    $errors"
  echo ""
  echo "Thumbnails are generated async. Check CloudWatch logs for mo-thumb."
fi
