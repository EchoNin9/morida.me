"""
MO API Lambda Handler
Main router for all morida.me API routes.
"""
...
TABLE_NAME = os.environ.get("TABLE_NAME", "mo-main")
MEDIA_BUCKET = os.environ.get("MEDIA_BUCKET", "mo-media")
COGNITO_USER_POOL_ID = os.environ.get("COGNITO_USER_POOL_ID", "")
THUMB_FUNCTION_NAME = os.environ.get("THUMB_FUNCTION_NAME", "mo-thumb")
...
def handle_media(event, method, parts):
    ...
    if method == "POST" and len(parts) >= 2 and parts[1] == "import-from-url":
        ...
        try:
            req = urllib.request.Request(source_url, headers={"User-Agent": "MO-Bot/1.0"})
...
def handler(event, context):
    ...
    if not parts:
        return ok({"service": "mo-api", "version": "1.0.0"})
